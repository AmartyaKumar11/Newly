  ...
  async function handleCreate() {
    ...
    try {
      ...
      try {
        ...
        // If navigation fails, the timeout will reset it as a fallback
        setTimeout(() => {
          setIsCreating(false);
        }, 1000);
      } catch (navError) {
        // Navigation failed, reset state immediately
        console.error("Navigation error:", navError);
        setIsCreating(false);
        alert("Newsletter created but navigation failed. Please refresh the page.");
      }
    } catch (error) {
